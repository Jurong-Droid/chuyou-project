<template>
  <div class="app-container">
    <div class="filter-container">
      <el-input size="medium" v-model="listQuery.name" style="width:8%;" @keyup.enter.native="handleFilter" placeholder="视频名" clearable />
      <el-input size="medium" v-model="listQuery.areaName" style="width:8%;" @keyup.enter.native="handleFilter" placeholder="位置" clearable />
      <el-cascader size="medium" v-model="listQuery.cameraId" :options="options" :props="defaultProps" filterable clearable placeholder="请选择摄像头信息" />
      <DatePicker style="height: 36px;" startVaule="异常开始日期" endValue="异常结束日期" @sendTimeData="getTime"></DatePicker>
    ...
    <div v-if="ismaker" style="width:50%; height: 50%; margin-left: 25%; position: fixed; top: 15%; left: 0; z-index: 9900;display: inline-block;zoom: 1.25">
      <div style="display: flex;flex-direction: column;position: absolute;margin-left: 90%;z-index: 9900;left: 20%;height: 30%;justify-content:space-around;width: 15%">
        <el-button type="danger" @click="ismaker=false" style="width: 80%">关闭</el-button>
        <el-button type="primary" @click="saveImage" style="width: 80%;margin-left: 0px">保存</el-button>
        <el-button size="mini" icon="el-icon-zoom-out" @click="zoomOut" style="width: 80%;margin-left: 0px">缩小</el-button>
        <el-button size="mini" icon="el-icon-zoom-in" @click="zoomIn" style="width: 80%;margin-left: 0px">放大</el-button>
        <el-select v-model="thetag" placeholder="选择标签" @change="setTag" :popper-append-to-body="false">
          <el-option v-for="item in detectTypes" :key="item.id" :label="item.name" :value="item.id"  style="margin-left: 30%; width: 80%;" />
        </el-select>
        <el-select v-model="theType" placeholder="选择模型类型" @change="setMoType" :popper-append-to-body="false">
          <el-option v-for="item in moduleTypes" :key="item.id" :label="item.name" :value="item.id"  style="margin-left: 30%; width: 80%;" />
        </el-select>
      </div>
      <ui-marker id="marker" style="transform: scale(1); transition: transform 0.2s;" ref="aiPanel-editor" class="ai-observer" :ratio="4/3" :imgUrl="imgUrl"></ui-marker>
    </div>

    <div>
      <el-table :data="list" v-loading="listLoading" element-loading-text="拼命加载中" border fit :cell-style="cellStyle" :header-cell-style="{background:'#f5f7fa',color:'#409EFF'}">
        <el-table-column align="center" label="序号" min-width="5">
          <template v-slot="scope">
            <span v-text="getIndex(scope.$index)"> </span>
          </template>
        </el-table-column>
        <el-table-column align="center" label="单位" prop="unit" min-width="8" />
        <el-table-column align="center" label="区域" prop="location" min-width="8" />
        <el-table-column align="center" label="报警类别" prop="name" min-width="15" />
        <el-table-column align="center" label="报警信息" min-width="27">
          <template v-slot="scope">
            <span> {{scope.row.expInfo}}</span>
          </template>
        </el-table-column>
        <el-table-column align="center" label="报警时间时间" prop="expTime" min-width="13" />

        <el-table-column align="center" label="处理情况" min-width="15">
          <template v-slot="scope">
            <span v-if="scope.row.closeBy !=null" style="color:green">已处置</span>
            <span v-else>未处置</span>
          </template>
        </el-table-column>
        <el-table-column align="center" label="处理信息" min-width="10">
          <template v-slot="scope">
            <span v-if="scope.row.dispose==-1" style="color: #18a9a5">未处理</span>
            <span v-else-if="scope.row.dispose==0" style="color:red ">误报</span>
            <span v-else-if="scope.row.dispose==1" style="color: green">实报</span>
          </template>
        </el-table-column>
        <el-table-column align="center" label="标注情况" min-width="10">
          <template v-slot="scope">
            <span v-if="scope.row.isTag==1" style="color: #18a9a5">已标注</span>
            <span v-else style="color: red">未标注</span>
          </template>
        </el-table-column>
        <el-table-column align="center" label="操作" min-width="15" v-permission="'abnormalInfo:start'">
          <template v-slot="scope">
            <el-button size="mini" type="warning" v-if="scope.row.closeBy ==null" @click="playMv(scope.row)">
  ...
  methods: {
    ...
    playMv(info) {
      ...
      if (process.env.NODE_ENV !== 'production') {
        this.video_url = process.env.BASE_URL + "/abnormalInfo/startAbnormalInfo?id=" +info.id;
      } else {
        this.video_url = window.location.origin + "/abnormalInfo/startAbnormalInfo?id=" +info.id;
      }
