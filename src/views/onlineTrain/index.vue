<template>
  <div class="app-container">
    <div v-if="showImage" style="width: 50%; height: 55%;margin-left: 10%;  position: fixed; top: 10%; z-index: 9900;">
      <el-button type="danger" @click="showImage=false">关闭</el-button>
      <img class="img" :src="ImageUrl">

    </div>
    <div v-if="setImage" style="width: 69%; height: 70%;margin-left: 10%;  position: fixed; top: 10%; z-index: 9900;zoom: 1.25">
        ...
        <el-select v-model="thetag" placeholder="选择标签" @change="setTag" :popper-append-to-body="false">
          <el-option v-for="item in detectTypes" :key="item.id" :label="item.name" :value="item.id" />
        </el-select>
        <ui-marker ref="aiPanel-editor" class="ai-observer"  :imgUrl="ImageUrl"></ui-marker>
    </div>
<!--    <div v-if="this.ModuleTypes[0].status!=-1">-->
      <el-select v-model="moduleType" placeholder="选择模型类型" :popper-append-to-body="false">
        <el-option v-for="item in ModuleTypes" :key="item.id" :label="item.name" :value="item.id"/>
      </el-select>
    <el-button size="mini" :plain='true' type="primary" style="font-size: 18px"  @click="toEdge()" >上传</el-button>
    <el-button size="mini" :plain='true' type="primary" style="font-size: 18px"  @click="toTrain()" >训练</el-button>
    <el-button size="mini" :plain='true' type="primary" style="font-size: 18px"  @click="download()" >下载</el-button>
    <el-button size="mini" :plain='true' type="primary" style="font-size: 18px"  @click="" >转化</el-button>
<!--    </div>-->
    <br>&nbsp;
    <el-table :data="abnormalList" element-loading-text="拼命加载中" border fit  :header-cell-style="{background:'#f5f7fa',color:'#409EFF'}">
      <el-table-column align="center" label="模型类型" min-width="8" >工装检测</el-table-column>
      ...
      <el-table-column align="center" label="标签类别" prop="tagName" min-width="8" />
      <el-table-column align="center" label="标注框数量" prop="tagNum" min-width="8"/>
      <el-table-column align="center" label="操作"  min-width="8" >
        <template v-slot="scope">
          <el-button size="mini" :plain='true' type="primary" style="font-size: 18px"  @click="openDimension(scope.$index)" >标注</el-button>
          <el-button size="mini" :plain='true' type="primary" style="font-size: 18px"  @click="openImage(scope.$index)" >查看</el-button>
        </template>
      </el-table-column>
    </el-table>
  ...
  methods:{
    download(){
      ...
      if (process.env.NODE_ENV !== 'production') {
        file_url = process.env.BASE_URL+"/onlineTrainInfo/getModuleFile?id="+this.moduleType;
      } else {
        file_url = window.location.origin +"/onlineTrainInfo/getModuleFile?id="+this.moduleType;
      }
      ...
      this.api({
        ...
        data.forEach(temp=>{
          if (this.abnormalLists!=[]){
            const trainData= this.abnormalLists.find(atemp=>atemp[0].id==temp.id);
            if (trainData!=null){
              trainData.push(temp)
            }else {
