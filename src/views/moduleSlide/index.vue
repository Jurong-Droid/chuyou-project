<template>
  <div class="app-container">
    <div style="display: flex; justify-content: space-between;">
      <div class="filter-container" :style="{width:showCameraList?'75%':'100%'}">
        <div>
          <el-select style="height:36px" size="medium" v-model="listQuery.edgeId" placeholder="请选择设备" clearable @change="handleSelectChange">
            <el-option v-for="item in edgeList" :key="item.id" :label="item.edgeName" :value="item.id"/>
          </el-select>
          <el-input  size="medium" v-model="listQuery.moduleName" style="width:10%;height:36px;" @keyup.enter.native="handleFilter"
                     placeholder="模型名称" clearable/>
          <el-button size="mini" v-waves type="primary" icon="el-icon-search" @click="handleFilter">搜索</el-button>
          <el-button size="mini" type="primary" style="width: 100px"  v-permission="'analyticsModule:add'" icon="el-icon-circle-plus" @click="showCreate"> 添加 </el-button>
        </div><br>
        <el-table :data="moduleList" border fit highlight-current-row style="font-size: 16px" :header-cell-style="{background:'#f5f7fa',color:'#409EFF'}" >
          <el-table-column align="center" label="序号" prop="id" min-width="3"></el-table-column>
          <el-table-column align="center" label="模型名称" prop="moduleName" min-width="8"></el-table-column>
          <el-table-column align="center" label="设备名称" prop="edgeName" min-width="8"></el-table-column>
          <el-table-column align="center" label="模型类型" prop="moduleType" min-width="8"></el-table-column>
          <el-table-column align="center" label="模型说明" prop="description" min-width="6"></el-table-column>
          <el-table-column v-if="this.edgeType == '0'"  align="center" label="绑定摄像头" min-width="8" >
            <template slot-scope="scope">
              <el-button  size="mini" type="primary" style="font-size: 18px" @click="opencameraList(scope.row.id)" >摄像头组</el-button>
            </template>
          </el-table-column>
          <el-table-column v-else align="center" label="绑定摄像头" min-width="8" prop="bdCamera">
            <template slot-scope="scope" >
              <el-button  size="mini" type="primary" style="font-size: 18px" @click="opencameraList(scope.row.id)" >列表</el-button>
            </template>
          </el-table-column>
          <el-table-column v-if="this.edgeType == '0'" align="center" label="最大摄像头检测量" prop="maxCameraNum" min-width="6"></el-table-column>
          <el-table-column  v-if="this.edgeType == '1'" align="center" label="模型运行时间" prop="runningTime" min-width="6"></el-table-column>
          ...
          <el-table-column align="center" label="版本" prop="version" min-width="4"></el-table-column>
          <el-table-column align="center" label="预处理" prop="opencv" min-width="5">
            <template slot-scope="scope">
              {{ scope.row.opencv === 0 ? '关闭' : '开启' }}
            </template>
          </el-table-column>
          <el-table-column align="center" label="最近修改时间" prop="updateTime" min-width="14"></el-table-column>
          <el-table-column align="center" label="管理" min-width="18">
            <template  v-slot="scope">
              ...
              <template  v-if="['2','6'].includes(scope.row.status)" >
                <el-button size="mini" :plain='true' type="primary" style="font-size: 18px" v-permission="'analyticsModule:newRelease'" @click="newSyncFile(scope.$index)" >同步</el-button>
              </template>
      ...
      <div class="filter-container" v-show="showCameraList" style="width: 25%;margin-top: 55px">

        <el-table v-if="this.edgeType == '1'" :data="cameraList" border fit highlight-current-row style="font-size: 16px" :header-cell-style="{background:'#f5f7fa',color:'#409EFF'}" >
          <el-table-column align="center" label="摄像头名称" prop="cameraName"></el-table-column>
        </el-table>
        <el-table v-else :data="cameraList" border fit highlight-current-row style="font-size: 16px" :header-cell-style="{background:'#f5f7fa',color:'#409EFF'}" >
          <el-table-column align="center" label="摄像头组号" prop="cgroupId" ></el-table-column>
          <el-table-column align="center" label="摄像头" prop="cameraName">
            <template slot-scope="scope">
              <div v-for="name in scope.row.cameraName">
                <div v-text="name" style="display: inline-block;vertical-align: middle;margin-top: 4%;"></div>
              </div>
            </template>
          </el-table-column>
          <el-table-column align="center" label="摄像头运行时间（秒）" prop="runTime"></el-table-column>
          <el-table-column align="center" label="操作">
            <template v-slot="scope">
              <el-button size="mini" v-waves type="primary" @click="ShowupdateGroup(scope.$index)">修改</el-button>
            </template>
          </el-table-column>
        </el-table><br>
        <div style="display: flex; justify-content: center; align-items: center;">
          <el-button size="mini" type="primary" @click="addcameraGroup">添加摄像头组</el-button>
          <el-button size="mini" type="primary" @click="openbindcamera">绑定摄像头</el-button>
        </div>
      </div>


      <el-dialog title="绑定摄像头" :visible.sync="dialogallCamera">
        <el-table  :data="allcamera" border fit highlight-current-row style="font-size: 16px" :header-cell-style="{background:'#f5f7fa',color:'#409EFF'}" >
          <el-table-column align="center" prop="id" label="摄像头ID" ></el-table-column>
          <el-table-column align="center" prop="cameraName" label="摄像头名称" ></el-table-column>
          <el-table-column align="center"  label="绑定" >
            <template v-slot="scope">
              <el-button size="mini" v-if="scope.row.status=='0'" v-waves type="primary" @click="bindcamera(scope.$index)" >绑定</el-button>
              <el-button size="mini" v-else v-waves type="primary" @click="unbindcamera(scope.$index)" >取消绑定</el-button>
            </template>
          </el-table-column>
        </el-table>
      </el-dialog>
      ...
      <el-dialog :title="textMap[dialogStatus]" :visible.sync="dialogFormVisible">
        <el-form class="small-space" :model="tempModule" label-position="left" label-width="18%">
          ...
          <el-form-item  v-if="this.edgeType == '1'" label="模型运行时间">
            <el-input type="text" v-model="tempModule.runningTime" style="width: 40%"></el-input>
          </el-form-item>
          <el-form-item v-if="this.edgeType=='0'" label="最大摄像头检测数量">
            <el-input type="text" v-model="tempModule.maxCameraNum" style="width: 40%"></el-input>
          </el-form-item>
          <el-form-item label="预处理状态" required>
            <el-select v-model="tempModule.opencv" placeholder="更改预处理状态" style="width: 40%">
              <el-option v-for="item in opencv" :key="item.id" :label="item.name" :value="item.id"/>
            </el-select>
          </el-form-item>
        </el-form>
      ...
      <el-dialog title="修改摄像头组" :visible.sync="dialogCamera" fullscreen  >
        <span style="font-size: 20px"> 摄像头组运行时间:</span> &nbsp;<el-input type="text" v-model="tempCamera.runTime" placeholder="摄像头组运行时间" style="width: 40%"></el-input>
        ...
        <div style="display: flex;">
          <div style="flex: 1;">
            <h1 style="text-align: center">摄像头组表({{this.hasNum+"/"+this.maxCameraNum}})</h1>
            <el-table :data="leftcameraList" border fit highlight-current-row style="font-size: 16px;" :header-cell-style="{background:'#f5f7fa',color:'#409EFF'}" >
              <el-table-column align="center" label="摄像头名" prop="cameraName" ></el-table-column>
              <el-table-column align="center" label="操作">
                <template v-slot="scope">
                  <el-button size="mini" v-waves type="primary" @click="deletecamera(scope.$index)">移出</el-button>
                </template>
              </el-table-column>
            </el-table>
          </div>
          ...
          <div style="flex: 1;">
            ...
            <el-table :data="rightcameraList" border fit highlight-current-style="font-size: 16px" :header-cell-style="{background:'#f5f7fa',color:'#409EFF'}" >
              <el-table-column align="center" label="摄像头名" prop="cameraName" ></el-table-column>
              <el-table-column align="center" label="操作">
                <template v-slot="scope">
                  <el-button size="mini" v-waves type="primary" @click="addcamera(scope.$index)">加入</el-button>
                </template>
              </el-table-column>
            </el-table>
          </div>
...
export default {
  ...
  data(){
    return{
      ...
      dialogCamera:false,
      maxCameraNum:"",
      opencv: [{id:1,name:'开启'},{id:0,name:'关闭'}],
      hasNum:"",
      listQuery: {
        pageNum: 1, //页码
        pageRow: 50, //每页条数
  ...
  methods:{
    ...
    getAllCameraList(id){//获取模型绑定的全部摄像头
      this.api({
    ...
    getCameraList(id){//查询摄像头
      ...
      if (this.edgeType=='0'){
        this.api({
          ...
          this.maxCameraNum=this.moduleList.find(temp=>temp.id==id).maxCameraNum
          this.api({
            ...
            this.cameraList = data.reduce((result, item) => {//按组存入this.cameraLIst进行归纳
              let group = result.find(group => group.cgroupId === item.cgroupId);
              if (group) {
            ...
            //将不在该模型内的摄像头存到右表
            this.rightcameraList = this.allcameraList.filter(camera => !this.cameraList.some(group => group.cameraId.includes(camera.id)));
            //按组id进行排序
    ...
    deletecamera($index){
      ...
      this.api({
        ...
        this.leftcameraList=this.leftcameraList.filter(temp=>temp!=camera)
      })
    },
    addcamera($index){
      ...
      this.api({
        ...
        console.log("添加成功")
        this.hasNum++;
        this.rightcameraList=this.rightcameraList.filter(temp=>temp.id!=camera.id)
        this.leftcameraList.push({
          moduleId:this.tempCamera.moduleId,
          cameraId:camera.id,
          cameraName:camera.cameraName,
    ...
    commandModule($index,command) {//发布
      let _vue = this;
    ...
    handleSelectChange(){
      ...
      if (this.listQuery.edgeId!='')
        this.edgeType =this.edgeList.find(temp=>temp.id==this.listQuery.edgeId).devicetype
    },
